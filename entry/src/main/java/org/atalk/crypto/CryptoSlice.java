...
import static org.atalk.ohos.gui.chat.ChatSlice.*;

/**
 * AbilitySlice when added to <code>Ability</code> will display the padlock allowing user to select
 * various type of encryption options. Only currently active chat is handled by this fragment.
 *
 * @author Eng Chong Meng
 */
public class CryptoSlice extends BaseSlice
    ...
    public void onCreateMenu(Menu menu, MenuInflater menuInflater) {
        ...
        mOmemo = menu.findComponentById(ResourceTable.Id_encryption_omemo);
    ...
    public boolean onMenuItemSelected(MenuItem menuItem) {
        ...
        switch (menuItem.getId()) {
            ...
            case ResourceTable.Id_encryption_none:
                if (mDescriptor instanceof Contact)
                    mChatType = ChatSlice.MSGTYPE_NORMAL;
                else
                    mChatType = ChatSlice.MSGTYPE_MUC_NORMAL;

                hasChange = true;
                doHandleOmemoPressed(false);
                break;

            case ResourceTable.Id_encryption_omemo:
    ...
    private MenuItem checkCryptoButton(int chatType) {
        ...
        switch (chatType) {
            ...
            case ChatSlice.MSGTYPE_OMEMO_UA:
            case ChatSlice.MSGTYPE_OMEMO_UT:
                mItem = mOmemo;
                break;
    ...
    private void doHandleOmemoPressed(final boolean enable) {
        ...
        if (mDescriptor instanceof Contact) {
            ...
            try {
                ...
                startAbility(OmemoAuthenticateDialog.createIntent(getContext(), mOmemoManager, omemoDevices, this));
                allTrusted = false;
                ...
                activeChat.addMessage(mEntity, new Date(), ChatMessage.MESSAGE_ERROR, IMessage.ENCODE_PLAIN,
                        getString(ResourceTable.String_crypto_msg_omemo_session_setup_failed, e.getMessage()));
            ...
            if ((numUntrusted > 0) && (numUntrusted == fingerPrints.size())) {
                ...
                activeChat.addMessage(mEntity, new Date(), ChatMessage.MESSAGE_SYSTEM, IMessage.ENCODE_PLAIN,
                        getString(ResourceTable.String_crypto_msg_omemo_session_untrusted));
            }
            else if (allTrusted) {
                mChatType = ChatSlice.MSGTYPE_OMEMO;
            }
            else {
                mChatType = ChatSlice.MSGTYPE_OMEMO_UA;
                activeChat.addMessage(mEntity, new Date(), ChatMessage.MESSAGE_SYSTEM, IMessage.ENCODE_PLAIN,
                        getString(ResourceTable.String_crypto_msg_omemo_session_unverified));
            }
        }
            ...
            if (allTrusted) {
                ...
                mChatType = ChatSlice.MSGTYPE_OMEMO_UA;
                activeChat.addMessage(mEntity, new Date(), ChatMessage.MESSAGE_SYSTEM, IMessage.ENCODE_PLAIN,
                        getString(ResourceTable.String_crypto_msg_omemo_session_unverified_untrusted));
            }
        }
    ...
    public void memberPresenceChanged(ChatRoomMemberPresenceChangeEvent evt) {
        if (mOmemoManager != null && (activeChat.isOmemoChat())
            ...
            try {
                ...
                Set<OmemoDevice> omemoDevices = e.getUndecidedDevices();
                Timber.w("There are undecided Omemo devices: %s", omemoDevices);
                startAbility(OmemoAuthenticateDialog.createIntent(mContext, mOmemoManager, omemoDevices, this));
            } catch (NoOmemoSupportException | InterruptedException | SmackException.NoResponseException
     ...
     * Must only be performed after the completion of onCreateMenu().
     *
...
//				encryptionChoice, mItem, activeChat.getChatType());
    ...
    private void setStatusOmemo(int chatType) {
        ...
        switch (chatType) {
            case ChatSlice.MSGTYPE_OMEMO:
                iconId = ResourceTable.Media_crypto_omemo_verified;
                tipKey = ResourceTable.String_omemo_menu_authenticated;
                break;
            case ChatSlice.MSGTYPE_OMEMO_UA:
                iconId = ResourceTable.Media_crypto_omemo_unverified;
                tipKey = ResourceTable.String_omemo_menu_unauthenticated;
                break;
            case ChatSlice.MSGTYPE_OMEMO_UT:
                iconId = ResourceTable.Media_crypto_omemo_untrusted;
                tipKey = ResourceTable.String_omemo_menu_untrusted;
                break;

            case MSGTYPE_NORMAL:
            case MSGTYPE_MUC_NORMAL:
                iconId = ResourceTable.Media_crypto_unsecure;
                tipKey = ResourceTable.String_menu_crypto_plain_text;
                break;
    ...
    public void onAuthenticate(boolean allTrusted, Set<OmemoDevice> omemoDevices) {
        if (allTrusted) {
            onOmemoAuthenticate(ChatSlice.MSGTYPE_OMEMO);
            activeChat.addMessage(mEntity, new Date(), ChatMessage.MESSAGE_SYSTEM, IMessage.ENCODE_PLAIN,
                    getString(ResourceTable.String_crypto_msg_omemo_session_verified));
        }
        else {
            onOmemoAuthenticate(ChatSlice.MSGTYPE_OMEMO_UA);
            // activeChat.addMessage(mEntity, new Date(), ChatMessage.MESSAGE_ERROR, IMessage.ENCODE_PLAIN,
    ...
    private void onOmemoAuthenticate(int chatType) {
        ...
        // ChatAbility onActive (on OmemoAuthenticateDialog closed) will trigger this. but do it just in case
        setStatusOmemo(chatType);
        updateOmemoSupport();
    }
