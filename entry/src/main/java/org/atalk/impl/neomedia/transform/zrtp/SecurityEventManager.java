...
public class SecurityEventManager extends ZrtpUserCallback
{
    ...
    public static final String WARNING_NO_RS_MATCH
            = aTalkApp.getResString(ResourceTable.String_media_security_warning_no_rs_match);

    /**
     * A warning <code>String</code> that we display to the user.
     */
    public static final String WARNING_NO_EXPECTED_RS_MATCH
            = aTalkApp.getResString(ResourceTable.String_media_security_warning_no_expected_rs_match);
    ...
    public void showMessage(ZrtpCodes.MessageSeverity sev, EnumSet<?> subCode)
    {
        ...
        if (msgCode instanceof ZrtpCodes.InfoCodes) {
            ...
            if (warn == ZrtpCodes.WarningCodes.WarningNoRSMatch) {
                ...
                i18nMessage = aTalkApp.getResString(ResourceTable.String_media_security_checksum_mismatch);
            }
            else {
            ...
            if (severe == ZrtpCodes.SevereCodes.SevereCannotSend) {
                ...
                i18nMessage = aTalkApp.getResString(ResourceTable.String_media_security_data_send_failed, msgCode.toString());
            }
            else if (severe == ZrtpCodes.SevereCodes.SevereTooMuchRetries) {
                message = "Too much retries during ZRTP negotiation.";
                i18nMessage = aTalkApp.getResString(ResourceTable.String_imedia_security_retry_rate_exceeded, msgCode.toString());
            }
            else if (severe == ZrtpCodes.SevereCodes.SevereProtocolError) {
                message = "Internal protocol error occurred.";
                i18nMessage = aTalkApp.getResString(ResourceTable.String_media_security_internal_protocol_error, msgCode.toString());
            }
            else {
                message = "General error has occurred.";
                i18nMessage = aTalkApp.getResString(ResourceTable.String_media_security_zrtp_generic_message, msgCode.toString());
            }
        }
    ...
    public void zrtpNegotiationFailed(ZrtpCodes.MessageSeverity severity, EnumSet<?> subCode)
    {
        ...
        aTalkApp.showToastMessage(ResourceTable.String_media_security_zrtp_handshake_timeout, msgCode);
