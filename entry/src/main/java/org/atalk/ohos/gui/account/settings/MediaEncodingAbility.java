...
/**
 * Ability allows user to edit audio or video encodings settings and set their priority.
 ...
 */
public class MediaEncodingAbility extends BaseAbility
        implements ActionBarToggleSlice.ActionBarToggleModel {
    /**
     ...
     */
    private MediaEncodingsSlice mEncodingsSlice;
    ...
    public void onStart(Intent intent) {
        ...
        loadEncodings(intent);

        if (mInstanceState == null) {
            ActionBarToggleSlice toggleFragment
                    = ActionBarToggleSlice.newInstance(getString(ResourceTable.String_enc_override_global));
    ...
    public void onSaveAbilityState(PacMap outState) {
        ...
        outState.putSerializableObject(STATE_ENC_REG, mEncReg);
        outState.putBooleanValue(STATE_HAS_CHANGES, hasChanges);
    }

    @Override
    public void onRestoreAbilityState(PacMap outState) {
        super.onRestoreAbilityState(outState);
        mInstanceState = outState;
    }

    /**
     * Loads properties passed by intent's extras and initializes the activity.
     * PacMap that contains the state or <code>null</code> if the <code>Ability</code> was just created.
     *
     * @param intent Ability Intent
     */
    private void loadEncodings(Intent intent) {
        if (mInstanceState == null) {
            mEncReg = intent.getSequenceableParam(EXTRA_KEY_ENC_REG);
        }
        else {
            mEncReg = (EncodingsRegistrationUtil) mInstanceState.getSerializable(STATE_ENC_REG).get();
            hasChanges = mInstanceState.getBooleanValue(STATE_HAS_CHANGES);
        }
        ...
        if (mediaServiceImpl != null) {
            ...
            mMediaType = intent.getSerializableParam(ENC_MEDIA_TYPE_KEY);
            if (mInstanceState == null) {
                List<MediaFormat> encodings = getEncodings(encodingConfiguration, mMediaType);
                ...
                mEncodingsSlice = MediaEncodingsSlice.newInstance(encodingsStrs, priorities);
                // getSupportFragmentManager().beginTransaction().replace(android.ResourceTable.Id_content, encodingsFragment).commit();
            }
            else {
                // mEncodingsSlice = (MediaEncodingsSlice) getSupportFragmentManager().findSliceById(ResourceTable.Id_content);
            }
            setMainRoute(mEncodingsSlice.getLocalClassName());
        }
    }
    ...
    public static List<Integer> getPriorities(List<MediaFormat> encodings,
        ...
        for (int i = 0; i < count; i++) {
            ...
            int orderPriority = MediaEncodingsSlice.calcPriority(encodings, i);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        mEncodingsSlice.setEnabled(isOverrideEncodings);
    ...
    protected void onBackPressed() {
        ...
        result.setParam(EXTRA_KEY_ENC_REG, mEncReg);

        if (mEncodingsSlice != null)
            hasChanges = hasChanges || mEncodingsSlice.hasChanges();
        result.setParam(EXTRA_KEY_HAS_CHANGES, hasChanges);
    ...
    public static void commitPriorities(EncodingConfiguration encodingConf, MediaType mediaType,
            MediaEncodingsSlice encFragment) {
    ...
    private void commitChanges() {
        commitPriorities(encodingConfiguration, mMediaType, mEncodingsSlice);
