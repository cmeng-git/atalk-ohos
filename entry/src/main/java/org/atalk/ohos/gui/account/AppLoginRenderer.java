...
import org.atalk.ohos.gui.authorization.AuthorizationHandlerImpl;
import org.atalk.ohos.gui.call.AppCallListener;
...
public class AppLoginRenderer implements LoginRenderer {
    ...
    /**
     * Local avatar PixelMap
     */
    private PixelMap localAvatar;
    ...
    /**
     * Local status PixelMap
     */
    private PixelMap localStatusPixelMap;
    ...
    public void protocolProviderConnectionFailed(final ProtocolProviderService protocolProvider,
        ...
        DialogH.getInstance(ctx).showConfirmDialog(aTalkApp.getInstance(),
                ResourceTable.String_error,
                ResourceTable.String_connection_failed_message,
                ResourceTable.String_retry,
    ...
    public void updateaTalkIconNotification() {
        ...
        if (getGlobalStatus().isOnline()) {
            ...
            status = aTalkApp.getResString(ResourceTable.String_online);

        }
        else {
            // There are no active providers, so we consider to be in the offline state
            status = aTalkApp.getResString(ResourceTable.String_offline);
        }
        ...
        AppUtils.updateGeneralNotification(aTalkApp.getInstance(), notificationID,
                aTalkApp.getResString(ResourceTable.String_app_name), status, System.currentTimeMillis());
    }
    ...
    /**
     * Returns the local user avatar pixelMap.
     *
     * @return the local user avatar pixelMap.
     */
    public PixelMap getLocalAvatarPixelMap(ProtocolProviderService provider) {
        ...
        // Re-create pixelMap only if avatar has changed
        if (avatarImage != localAvatarRaw) {
            localAvatarRaw = avatarImage;
            localAvatar = AppImageUtil.getCircularPixelMapFromBytes(avatarImage);
        }
        return localAvatar;
    }

    /**
     * Returns the local user status pixelMap.
     *
     * @return the local user status pixelMap
     */
    synchronized public PixelMap getLocalStatusPixelMap() {
        byte[] statusImage = StatusUtil.getContactStatusIcon(getGlobalStatus());
        if (statusImage != localStatusRaw) {
            localStatusRaw = statusImage;
            localStatusPixelMap = (localStatusRaw != null)
                    ? AppImageUtil.pixelMapFromBytes(statusImage) : null;
        }
        return localStatusPixelMap;
    }
}
