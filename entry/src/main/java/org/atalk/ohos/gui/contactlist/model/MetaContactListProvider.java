 ...
 * IllegalStateException: The content of the adapter has changed but ListContainer did not receive a notification.
 ...
 */
public class MetaContactListProvider extends BaseContactListProvider
    ...
    public MetaContactListProvider(ContactListSlice contactListFragment, boolean mainContactList) {
    ...
    public void contactBlockingStatusChanged(Contact contact, boolean blockState) {
        ...
        if (groupIndex >= 0) {
            int contactIndex = getChildIndex(getContactList(groupIndex), metaContact);
            if (contactIndex >= 0) {
                BaseAbility.runOnUiThread(() -> updateBlockStatus(groupIndex, contactIndex, contact));
            }
        }
    }
    ...
    public void metaContactRenamed(final MetaContactRenamedEvent evt) {
        ...
        BaseAbility.runOnUiThread(() -> updateDisplayName(evt.getSourceMetaContact()));
    }
    ...
    public void protoContactAdded(final ProtoContactEvent evt) {
        ...
        BaseAbility.runOnUiThread(() -> updateStatus(evt.getNewParent()));
    }
    ...
    public void protoContactRemoved(final ProtoContactEvent evt) {
        ...
        BaseAbility.runOnUiThread(() -> updateStatus(evt.getOldParent()));
    }
    ...
    public void metaContactAvatarUpdated(final MetaContactAvatarUpdateEvent evt) {
        ...
        BaseAbility.runOnUiThread(() -> updateAvatar(evt.getSourceMetaContact()));
    }
