...
import com.pierfrancescosoffritti.youtubeplayer.player.views.YouTubePlayerView;
...
public class YoutubePlayerSlice extends BaseSlice {
    ...
    public void onStart(Intent intent) {
        ...
        mContext = (ChatAbility) getContext();
        fullScreenHelper = new FullScreenHelper(this);

        LayoutScatter inflater = LayoutScatter.getInstance(getContext());
        Component view = inflater.parse(ResourceTable.Layout_youtube_player_slice, null, false);
        youTubePlayerView = view.findComponentById(ResourceTable.Id_youtube_player_view);

        mediaUrl = intent.getStringParam(MediaPlayerSlice.ATTR_MEDIA_URL);
        mediaUrls = intent.getStringArrayListParam(MediaPlayerSlice.ATTR_MEDIA_URLS);
    ...
    private void initYouTubePlayerView() {
        ...
        youTubePlayerView.addYouTubePlayerListener(new AbstractYouTubePlayerListener() {
            ...
            public void onPlaybackRateChange(YouTubePlayer youTubePlayer, String rate) {
                ...
                aTalkApp.showToastMessage(ResourceTable.String_playback_rate, rate);
            }
        });
    }
    ...
    private void startPlaylist(YouTubePlayer youTubePlayer, String videoId) {
        PixelMap nextActionIcon = AppImageUtil.getPixelMap(mContext, ResourceTable.Media_ayp_ic_next);
    ...
    private void addActionsToPlayer(YouTubePlayer youTubePlayer) {
        PixelMap rewindActionIcon = AppImageUtil.getPixelMap(mContext, ResourceTable.Media_ayp_ic_rewind);
        PixelMap forwardActionIcon = AppImageUtil.getPixelMap(mContext, ResourceTable.Media_ayp_ic_forward);
        PixelMap offYoutubeActionIcon = AppImageUtil.getPixelMap(mContext, ResourceTable.Media_ayp_ic_hide_screen);
        PixelMap rateIcon = AppImageUtil.getPixelMap(mContext, ResourceTable.Media_ayp_ic_speed);
    ...
    private void initPlayerMenu() {
        Objects.requireNonNull(youTubePlayerView.getPlayerUiController()
                        .showMenuButton(true)
                        .getMenu())
                .addItem(new MenuItem("menu item1", ResourceTable.Media_ayp_ic_speed_inc,
                        view -> aTalkApp.showToastMessage("item1 clicked"))
                )
                .addItem(new MenuItem("menu item2", ResourceTable.Graphic_ic_mood_black_24dp,
                        view -> aTalkApp.showToastMessage("item2 clicked"))
                )
                .addItem(new MenuItem("menu item no icon", -1,
                        view -> aTalkApp.showToastMessage("item no icon clicked"))
                );
    }
    ...
    public void initPlaybackSpeed(YouTubePlayer youTubePlayer) {
        mSpeed = (float) configService.getDouble(MediaPlayerSlice.PREF_PLAYBACK_SPEED, 1.0);
    ...
    public void release() {
        ...
        if (mSpeed >= rateMin && mSpeed <= rateMax) {
            configService.setProperty(MediaPlayerSlice.PREF_PLAYBACK_SPEED, mSpeed);
        }
        youTubePlayerView.release();
    }

    public void setPlayerVisible(boolean show) {
        youTubePlayerView.setVisibility(show ? Component.VISIBLE : Component.HIDE);
    }

    public boolean isPlayerVisible() {
        return youTubePlayerView.getVisibility() == Component.VISIBLE;
    }
}
